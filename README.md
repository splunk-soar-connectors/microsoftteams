...
- Messages and reactions in Microsoft Teams will appear to come from the human user who configured the Microsoft Entra ID app. To avoid this, create a new Active Directory user to serve as the "bot", and use it to configure the app.
- In previous versions of the connector, it was possible to use an Azure Bot identity to configure the connector, instead of an AD user. This is no longer supported, as Microsoft has deprecated the underlying API. This also means that the "ask question" action is no longer supported for new deployments.
- As a substitute for the "ask question" action, you can use an [External prompt block](https://help.splunk.com/en/splunk-soar/soar-on-premises/build-playbooks/7.1.0/use-the-playbook-editor-to-create-and-view-playbooks-to-automate-analyst-workflows/require-user-input-using-the-prompt-block-in-your-splunk-soar-on-premises-playbook#ariaid-title3) with one of the following actions:
...
- Select **Microsoft Entra ID** from the left side menu
...
[ask question](#action-ask-question) - Ask a question in a channel (DEPRECATED) <br>
...
Ask a question in a channel (DEPRECATED)
